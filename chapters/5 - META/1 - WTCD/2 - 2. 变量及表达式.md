...
- `string`：字符串
- `null`：空值
- `action`： 表示一个需要可执行的操作（`goto` 或者 `exit`）
...
### 2.3.3 变量的作用域
WTCD 使用[静态作用域](https://zh.wikipedia.org/wiki/%E4%BD%9C%E7%94%A8%E5%9F%9F)（[Lexical Scoping](https://en.wikipedia.org/wiki/Scope_(computer_science)#Lexical_scoping)）。每一个由大括号包裹的**语句组**将组成一个新的作用域（语句组将在 **2.4 ）。// TODO here!
