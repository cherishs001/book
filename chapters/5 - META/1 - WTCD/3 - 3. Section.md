[评论](https://github.com/SCLeoX/Wearable-Technology/issues/73)
...
WTCD 会根据每一个逻辑块的后发表达式运算结果来决定需要做什么。后发表达式的返回值必须是 `action`（见 3.2.2.1），`selection`（见 3.2.2.2），或者 `null`（见 3.2.2.1.2）。如果类型是 `action`，那么这个 `action` 就会被执行。如果类型是 `selection`，那么 WTCD 就会让用户做出选择，然后执行用户选择相对应的 `action`。如果是 `null`，则会什么也不做。

#### 3.2.2.1 Action
Action 指的是可以立刻完成的操作。目前 WTCD 支持 2 种 action：`goto` 和 `exit`。
...
##### 3.2.2.2.1 创建 Selection 和 Choice
...
#### 3.2.2.3 Action，Choice 和 Selection 作为值
这里需要注意的是，Action，Choice，和 Selection 都是可以存储的值。它们各自的类型分别是：`action`，`choice`，和 `selection`。
...
declare [
  ...
  selection someSelection = selection [ someChoice1 someChoice2 ]
]
