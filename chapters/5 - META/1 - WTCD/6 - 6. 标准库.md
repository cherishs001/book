...
接受 4 个参数：`string id`，`string path`，`number x`，`number y`。

将位于 `path` 的图片绘制到 ID 为 `id` 的画布上。使用 `x` 和 `y` 可以控制绘制的位置（左上角）。
