...
同时，请务必注意，在规范的写作里是几乎没有纯换行的（换行，但是不进入下一个自然段）。因此，Markdown 不支持纯换行。所以请永远不要只使用一个回车。**比如下述例子就是错误的，它渲染出来的结果是两行文字会出现在同一行，完全没有换行。**
...
### 链接
有时候你可能会希望添加超链接链接。在 Markdown 里，超链接的格式长这样：`[链接的文字](链接网址)`。举个例子：
