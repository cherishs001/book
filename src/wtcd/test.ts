...
section [ initialShop shop ]
    then goto purchaseList

section purchaseList then selection [
    (money >= 20)
        ? choice "购买春药（20 元）" goto purchaseAphrodisiac
        : choice "购买春药（需要 20 元）" disabled
    (vibrator)
        ? null
        : (money >= 15)
            ? choice "购买跳蛋（15 元）" goto purchaseVibrator
            : choice "购买跳蛋（需要 15 元）" disabled
    (vaginalDildo)
        ? null
        : (money >= 20)
            ? choice "购买阴道按摩棒（20 元）" goto purchaseVaginalDildo
            : choice "购买阴道按摩棒（需要 20 元）" disabled
    (analDildo)
        ? null
        : (money >= 20)
            ? choice "购买肛门按摩棒（20 元）" goto purchaseAnalDildo
            : choice "购买肛门按摩棒（需要 20 元）" disabled
    (chastityBelt)
        ? null
        : (money >= 50)
            ? choice "购买贞操带（50 元）" goto purchaseChastityBelt
            : choice "购买贞操带（需要 50 元）" disabled
    choice "离开商店" goto exitShop
]

section purchaseAphrodisiac changes [
    money -= 10
    aphrodisiac++
...
section purchaseVibrator changes [
    money -= 15
    vibrator = true
] then goto shop

section purchaseVaginalDildo changes [
    money -= 20
    vaginalDildo = true
] then goto shop

section purchaseAnalDildo changes [
    money -= 20
    analDildo = true
] then goto shop

section purchaseChastityBelt changes [
    money -= 50
    chastityBelt = true
] then goto shop
...
parse(source2, new MarkdownIt({ html: true }), console);
